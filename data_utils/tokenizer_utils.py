...
def create_tokenizer(path, return_pretokenized: bool = False, tokenizer_ckpt: str = None):
    ...
    if return_pretokenized:
        ...
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_ckpt, clean_up_tokenization_spaces=True)
        return tokenizer
...
def read_byte_level(path: str):
    ...
    snt = "Bores can be divided into two classes; those who have their own particular subject, and those who do not need a subject."

    print(
        tokenizer.encode(snt).ids
    )
...
def to_aa_token_seq(tokens):
    for i in range(1, len(tokens) - 1):
        if not tokens[i + 1].startswith("Ġ") and tokens[i + 1] != '</s>':
            tokens[i] += "@@"

...
from transformers import BertModel, BartModel, AutoTokenizer, AutoModel
...
def shift_tokens_right(input_ids: th.Tensor, pad_token_id: int, decoder_start_token_id: int):
    """
...
def get_decayed_weight(current, total, weight_scale):
    weight = max(0, math.cos(current / (total * weight_scale) * math.pi))
    return weight
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        if config.model.use_teacher:
            self.teacher_model = AutoModel.from_pretrained(config.model.teacher).to(config.device)
    ...
    def training_losses(self, model, input_text, t, is_dev=False, current_train_step=0, total_train_step=100):
        if self.training_mode == 's2s':  # choose
            return self.training_losses_s2s(model, input_text, t, is_dev=is_dev, current_train_step=current_train_step, total_train_step=total_train_step)
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                yield out
                text = out["sample"]
                # TODO: 1. langvifn。2. paraGuide，logit，classifier，text
    ...
    def training_losses_s2s(self, model, input_text, t, is_dev=False, current_train_step=0, total_train_step=100):
        """
        ...
        # print("current t:", t.shape)    # t shape: (batch_size, tgt_len)
        # print(t[:, -1] / self.config.diffusion_steps)
        ...
        # prepare src_head_mask
        dp_head_masks = None
        if self.config.use_src_dp:
            src_head_mask = input_text['src_dp_mask'].long().unsqueeze(1).to(t.device)
            batch_size, num_head, seq_len, seq_len = src_head_mask.shape
            dp_head_masks = th.ones(6, batch_size, num_head, seq_len, seq_len, device=src_head_mask.device, dtype=src_head_mask.dtype)
            dp_head_masks[self.config.dp_head_layer] = src_head_mask
        ...
        if self.loss_type == LossType.E2E_MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.config.self_condition and random() < 0.5:
                with th.no_grad():
                    prev_output, length_out = model(tgt_emb=x_t, 
                                                    ...
                                                    src_head_mask=dp_head_masks)    # add src dp head masks during training
            ...
            # calculate MSE using a pretrained BERT
            if self.config.model.use_teacher:
                if self.config.model.context_aware:
                    # Works
                    # print("using bert last hidden as signal")
                    tgt_bert_hidden = self.teacher_model(q_input_ids).last_hidden_state
                    tgt_bert_hidden = model.bert2diff(tgt_bert_hidden)
                    terms["bert_mse"] = ((tgt_bert_hidden - x_start) ** 2).mean(-1) * self.config.model.teacher_weight * self.config.model.teacher_lambda
                elif self.config.model.logits_aware:
                    # Failed
                    # print("using bert logits as signal")
                    diffuseq_E_matrix = model.word_embedding.weight.t()  # (hidden_size, vocab_size)
                    diffuseq_logits = th.matmul(model_out_x_start, diffuseq_E_matrix)  # (bsz, len, vocab_size)
                    diffuseq_probs = F.softmax(diffuseq_logits, dim=-1)  # (bsz, len, vocab_size)

                    bert_E_matrix = self.teacher_model.embeddings.word_embeddings.weight  # (vocab_size, hidden_size)
                    bert_diffuseq_embed = th.matmul(diffuseq_probs, bert_E_matrix)  # (bsz, len, hidden_size)

                    bert_diffuseq_output = self.teacher_model(inputs_embeds=bert_diffuseq_embed, 
                                                              attention_mask=q_attention_mask).last_hidden_state  # (bsz, len, hidden_size)
                    bert_target_output = self.teacher_model(input_ids=q_input_ids, 
                                                            attention_mask=q_attention_mask).last_hidden_state  # (bsz, len, hidden_size)

                    # bert_diffuseq_output = th.mean(bert_diffuseq_output, dim=1)  # (bsz, hidden_size)
                    # bert_target_output = th.mean(bert_target_output, dim=1)  # (bsz, hidden_size)\
                    terms["bert_mse"] = F.mse_loss(bert_diffuseq_output, bert_target_output, reduction='none').mean(-1) # (bsz, len)
                else:
                    # Works
                    # print("using bert embeddings as signal")
                    bert_emb = self.teacher_model.embeddings.word_embeddings(q_input_ids)
                    bert_emb = model.bert2diff(bert_emb)
                    
                    terms["bert_mse"] = ((bert_emb - x_start) ** 2).mean(-1) * self.config.model.teacher_weight * self.config.model.teacher_lambda

            # use aux ar model to get reconstruction loss
            if self.config.model.aux_ar_model:
                if self.config.model.reconstruct_tgt == "x":
                    labels = th.where(p_input_ids == 0, -100, p_input_ids)
                    aux_output = model.aux_ar_model(
                        inputs_embeds=model_output,
                        labels=labels,
                    )
                    scale = model_output.size(0) * model_output.size(1)
                    terms["aux_ar_loss"] = aux_output.loss / scale * self.config.model.aux_ar_weight * self.config.model.aux_ar_lambda
                if self.config.model.reconstruct_tgt == "y":
                    labels = th.where(q_input_ids == 0, -100, q_input_ids)
                    aux_output = model.aux_ar_model(
                        inputs_embeds=model_output,
                        labels=labels,
                    )
                    scale = model_output.size(0) * model_output.size(1)
                    terms["aux_ar_loss"] = aux_output.loss / scale * self.config.model.aux_ar_weight
                if self.config.model.reconstruct_tgt == "xy":
                    labels = th.concat((p_input_ids, q_input_ids), dim=-1)
                    labels = th.where(labels == 0, -100, labels)
            ...
            reg_weight = get_decayed_weight(current_train_step, total_train_step, self.config.reg_weight_scale) if self.config.use_decay_reg_weight else 1
            if current_train_step % self.config.eval_interval == 0 and not is_dev:
                print(reg_weight, current_train_step, total_train_step)
            ...
            if "vb" in terms:
                ...
                if self.config.pred_len:
                    ...
                    if self.config.model.use_teacher:
                        if self.config.schedule_sampler == 'uniform':
                            terms["bert_mse"] = terms["bert_mse"].mean(-1)
                        terms["loss"] += terms["bert_mse"] * reg_weight

                    if self.config.model.aux_ar_model:
                        if self.config.schedule_sampler == 'uniform':
                            terms["aux_ar_loss"] = terms["aux_ar_loss"].mean(-1)
                        terms["loss"] += terms["aux_ar_loss"] * reg_weight

                    if self.config.use_reg_random:
                        rand_emb = th.randn_like(model_output)
                        terms["reg_random"] = ((rand_emb - x_start) ** 2).mean(-1)
                        terms["loss"] += terms["reg_random"] * reg_weight

                    if self.config.use_reg_l2:
                        terms["reg_l2"] = (x_start ** 2).mean(-1)
                        terms["loss"] += terms["reg_l2"] * reg_weight

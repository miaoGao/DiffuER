...
LOGITS_AWARE=False
USE_AUX_AR_MODEL=True
RECONSTRUCT_TGT=x
USE_REG_L2=False
USE_REG_RANDOM=False
use_decay_reg_weight=False
reg_weight_scale=2         # param in weight_decay func
...
exp_root=my_output_qqp/$DIFFU_SAMPLER\_$DATA_NAME\_aux_ar$USE_AUX_AR_MODEL\_bert_mse$USE_TEACHER\_teacher_lambda$TEACHER_LAMBDA\_aux_ar_lambda$AUX_AR_LAMBDA
...
if [ $do_train ]; then
    ...
    model.use_teacher=$USE_TEACHER model.teacher=$TOKENIZER model.context_aware=$CONTEXT_AWARE model.logits_aware=$LOGITS_AWARE \
    model.time_weighted=$TIME_WEIGHTED \
    use_decay_reg_weight=$use_decay_reg_weight reg_weight_scale=$reg_weight_scale \
    visible_device=$visible_device \
    model.teacher_lambda=$TEACHER_LAMBDA model.aux_ar_lambda=$AUX_AR_LAMBDA
fi
...
if [ $do_gen ]; then
    ...
    data.name=qqp max_pos_len=$max_pos_len num_samples=$num_samples \
...
if [ $do_gen_all_cp ]; then
    num_samples=20
    for ((i = 10000; i <=$STEP; i += 10000)); do
        echo $i
        python ./gen_utils/m_generate.py \
        model.name=$MODEL_NAME model.tokenizer=$TOKENIZER \
        exp.name=$FILE_NAME exp.root=$exp_root load_step=$i \
        data.name=$FILE_NAME max_pos_len=$max_pos_len num_samples=$num_samples \
        intermediate_size=512 num_attention_heads=8 \
        in_channels=64 out_channels=64 time_channels=64 \
        skip_sample=True gen_timesteps=$gen_timesteps skip_timestep=$skip_timestep \
        schedule_sampler=$DIFFU_SAMPLER time_att=True att_strategy='txl' \
    ...
    if [ $DIFFU_SAMPLER == uniform ]; then
        SAMPLER_NAME=un
        folder=$exp_root/$DATA_NAME/$DATA_NAME/$i\_ema_0.9999_skip__$SAMPLER_NAME\_$skip_timestep
    fi

    if [ $DIFFU_SAMPLER == xy_uniform ]; then
        SAMPLER_NAME=xy
        folder=$exp_root/$DATA_NAME/$DATA_NAME/$i\_ema_0.9999_skip__$SAMPLER_NAME\_$gen_timesteps
    fi
    ...
    files2rouge $pred_file $ref_file --ignore_empty_reference --ignore_empty_summary >> $exp_root/$DATA_NAME/$DATA_NAME/log.log

    ### Evaluating
    python eval_utils/evaluation/eval.py --folder $folder --device_idx $visible_device >> $exp_root/$DATA_NAME/$DATA_NAME/log.log
    done
fi

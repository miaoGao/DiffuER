            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if self.dev_data is not None and self.global_step % self.eval_interval == 0:
                    ...
                    for step, batch in tqdm(enumerate(self.dev_data)):
                        ...
                        batch_losses = self.forward_only(step, batch)
                        for key, item in batch_losses.items():
                            if eval_losses.__contains__(key):
                                eval_losses[key] += item.item()
                            else:
                                eval_losses[key] = item.item()
                    for key, item in eval_losses.items():
                        logger.info(f"{key}: {item / num_batch}")
        ...
        if self.train_type == 's2s':
            ...
            if self.config.use_AMP:
                with autocast(device_type='cuda', dtype=torch.float16):
                    losses = self.diffusion.training_losses(self.model, batch, t, current_train_step=self.global_step, total_train_step=self.total_steps)
    ...
    def forward_only(self, step, batch):
        with torch.no_grad():
            ...
            if self.train_type == 's2s':
                ...
                if self.config.use_AMP:
                    with autocast(device_type='cuda', dtype=torch.float16):
                        losses = self.diffusion.training_losses(self.model, batch, t, is_dev=True, current_train_step=self.global_step, total_train_step=self.total_steps)
                else:
                    losses = self.diffusion.training_losses(self.model, batch, t, is_dev=True, current_train_step=self.global_step, total_train_step=self.total_steps)
            else:
                return NotImplementedError
